...
- Компоненты в секции "модули" при инициализации также догружают модули с Яндекса. Модули грузятся через `ymaps3.import`
  и регулируются самим Яндексом - библиотека лишь упрощает их подключение. **Присутствуют также компоненты, которые грузятся через CDN**
  - [Узнать больше можно тут](cdn)
...
- Координаты у Яндекса находятся не в том порядке, в котором были в старых версиях: первым идет не широта и долгота, а долгота и широта
