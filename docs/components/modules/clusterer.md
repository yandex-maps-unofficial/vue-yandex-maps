...
- `zoomCorrectDuration` (по умолчанию: `duration` или 500)
- `zoomCorrectEasing` (по умолчанию: `easing` или ничего)
- `strategy`:
    - `zoomCorrect` (по умолчанию): после первоначальной центровки на границы значение `zoom` дополнительно корректируется в целочисленное значение (в меньшую сторону).
