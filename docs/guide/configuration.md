...
Селектор для определения, что компонент находится внутри карты.

По умолчанию: `ymaps,ymaps3`.

Установите в false, чтобы отключить это поведение - однако, это может привести к дубликатам компонентов и другим поломкам, особенно маркеров.

### typesInjection

::: info Информация
Настройка доступна только при подключении модуля Nuxt
:::
