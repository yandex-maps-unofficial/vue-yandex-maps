...
Очень сильно поменялся `YandexMapDefaultMarker`. Это самое жирное изменение, которое потребует апгрейда.
...
- 🟥 `YandexMapDefaultMarker` упразднён в пользу компонента, который ранее назывался `YandexMapUiMarker` (гайд по миграции
  находится выше)
  -  `YMapDefaultMarkerProps`
- 🟥 `YandexMapSphericalMercatorProjection` переименован в `YandexMapWebMercatorProjection` без каких-либо изменений - теперь он также грузит CDN-модуль `@yandex/ymaps3-web-mercator-projection`
