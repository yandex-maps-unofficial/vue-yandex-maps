...
- Из зависимостей удалён `@nuxt/kit` - он по идее и так есть у всех, кто использует Nuxt 3, незачем его ставить
  остальным
- Улучшены возможности по установке breaking-версий `@yandex/ymaps3-types`: теперь можно установить любую версию до
  1.x (если Яндекс до неё дойдёт)
