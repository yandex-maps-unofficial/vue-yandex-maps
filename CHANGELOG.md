...
Данная версия является первым обновлением с новыми фичами с релиза и добавляет поддержку методов Яндекса, завязанных на указании токенов.
...
- Судя по всему, была добавлена функция `setCamera` в модель YandexMap
- Добавлена настройка `transparent` в `YMapControl` (делает компонент прозрачным без фона и теней)
    ...
    - На данный момент эта настройка ничего не делает)) Просто ничего не происходит. Ждём обновления от Яндекса
